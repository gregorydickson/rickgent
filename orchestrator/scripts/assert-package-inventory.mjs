...
function matchesAny(path, patterns) {
  return patterns.some((pattern) => {
    ...
    // A bare token matches a path segment; a dotted token also matches a file
    // suffix so generated bytecode and build metadata cannot hide below a
    // nested package directory.
    if (!pattern.includes("/")) {
      return path.split("/").some((segment) =>
        segment === pattern || (pattern.startsWith(".") && segment.endsWith(pattern))
      );
    }
...
function main() {
  ...
  for (const required of npmMustInclude) {
    const present = required.endsWith("/")
      ? npmFiles.some((path) => path.startsWith(required))
      : npmFiles.includes(required);
    if (!present) {
