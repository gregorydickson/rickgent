...
import {
  ...
  ATTEMPT_CLEANUP_PROOF_STATE_SQLITE_SCHEMA_CHECKSUM,
  ATTEMPT_LEGAL_EDGE_FAILURE_MIGRATION_CHECKSUM,
...
describe("frozen state contract parity", () => {
  it("matches metadata, migrations, tables, transactions, and SQLite settings", () => {
    ...
    expect(ATTEMPT_CLEANUP_PROOF_MIGRATION.sqlite_schema_checksum).toBe(ATTEMPT_CLEANUP_PROOF_STATE_SQLITE_SCHEMA_CHECKSUM);
    expect(EXECUTABLE_STATE_MIGRATIONS[5]).toMatchObject({
      version: ATTEMPT_LEGAL_EDGE_FAILURE_MIGRATION.version,
      number: ATTEMPT_LEGAL_EDGE_FAILURE_MIGRATION.number,
      name: ATTEMPT_LEGAL_EDGE_FAILURE_MIGRATION.name,
      checksum: ATTEMPT_LEGAL_EDGE_FAILURE_MIGRATION_CHECKSUM,
    });
    expect(ATTEMPT_LEGAL_EDGE_FAILURE_MIGRATION.released_checksum).toBe(ATTEMPT_LEGAL_EDGE_FAILURE_MIGRATION_CHECKSUM);
    expect(ATTEMPT_LEGAL_EDGE_FAILURE_MIGRATION.sqlite_schema_checksum).toBe(LATEST_STATE_SQLITE_SCHEMA_CHECKSUM);
