...
- **Chained/prefixed-command bypass (orthogonal review, CRITICAL):** the protective force-push DENY is also evadable by command prefixes — `cd sub && git push --force origin main`, `sudo git push …`, `; git push …`, `git -C dir push --force …`. A denylist that anchors on a leading `git push` misses all of these. The structural parser MUST split on shell separators (`&&`, `||`, `|`, `;`, `&`, newline) and evaluate every segment, and MUST normalize prefixes (`sudo`, env-assignment `FOO=1`, `git -C <dir>`, `cd … &&`) before matching — the same segmentation discipline Mission 1's `config-protection.ts` uses (`splitShellSegments`). This applies symmetrically to A-SEC-2's abstain logic.
- **Fix:** parse the push into (remote, refspec, flags) structurally, across all shell segments and after prefix normalization; resolve the concrete destination ref; reject the enumerated shapes above; require the resolved dest to equal the run's ticket feature branch. Do not rely on substring matching of the raw command, and do not anchor on the command's leading token.
- **AC:** M2-AC-SEC-1.
...
- **Second, distinct fail-open mechanism — tool-NAME coverage (orthogonal review, CRITICAL):** independent of the command-content denylist, the fence only matches a fixed set of tool names. It **fails open for the `shell` / `Shell` tool-name variants** that non-Omnigent harnesses emit (e.g. Cursor's `Shell`, Pi's lowercase `shell`). For those harnesses the fence never evaluates at all — every write is unscoped. This matters directly for goal 1 (multi-model): the moment B8 routes to a non-`claude`/`codex` harness, the scope fence silently stops applying. The tool-name match set MUST cover every harness's shell/write tool names (cross-reference Mission 1's `blast_radius` match set: `sys_os_shell`, `Bash`, `bash`, `Shell`, `shell`, plus each harness's structured write tools).
- **False-positive direction (MEDIUM):** the current heuristic also flags read-only redirects (`2>&1`, a quoted `'>'`) as writes — fix while reworking, so the corrected fence neither fails open on real writes nor false-DENYs benign redirects.
- **Why it matters:** the shell scope fence provides **zero real path-scoping** — bypass → ALLOW is trivial via content OR tool-name, and the detected path always DENYs. It only constrains the structured `Write`/`Edit` tools of the harnesses it recognizes. Breaks goals 2 and 3 (and goal 1 via the tool-name gap).
...
- **The allowlist is also partly fiction (orthogonal review, HIGH):** `ALLOWED_COMPLETION_CALLERS` enumerates 7 caller ids, but 6 of the 7 are not real call sites anywhere in the codebase — they are aspirational names for callers that don't exist yet (the lifecycle that would call them is stubbed, Workstream B). And the AC-5 test (`caller-audit.test.ts`) verifies only that a *rogue* caller throws; it never verifies that the *real* completion path actually routes through `evaluateCompletion` with an allowlisted caller. So the "pin" currently guards nothing: there is no live authorized caller to protect, and the test would pass against an implementation that never calls the oracle at all.
- **Required behavior:** make `caller` required (non-optional, typed as a union/branded type of the allowed caller ids). Every call site must pass its identity. As Workstream B wires the real lifecycle, the allowlist must be trimmed to callers that actually exist and each must be exercised. The AC-5 static-import-graph audit must additionally assert (a) there is no call to `evaluateCompletion` lacking a caller argument, and (b) every id in `ALLOWED_COMPLETION_CALLERS` corresponds to a real call site (no phantom entries), and (c) the production completion path routes through the oracle (a positive routing test, not only the negative rogue-caller throw).
- **Fix:** change the signature to `caller: CompletionCaller`, remove the `!= null` short-circuit (`if (!ALLOWED_COMPLETION_CALLERS.has(caller)) throw`), update all call sites, trim phantom allowlist entries as B lands, extend the AC-5 audit per (a)/(b)/(c).
- **AC:** M2-AC-SEC-6.
...
#### A-BUG-9 — Ticket lock default staleness (5s) lets two workers run the same ticket
- **File:** `orchestrator/src/dispatch/dispatch.ts` (`TicketLock` staleness default, near `acquire`)
- **Current:** the stale-lock takeover default is ~5 seconds. A worker legitimately holding a ticket for longer than 5s (i.e. essentially always — real work takes minutes) has its lock "stale-stolen" by a second dispatch, so two workers run the same ticket concurrently. (Orthogonal review, MEDIUM; distinct from A-BUG-4's NaN-wedge — that is "never steals", this is "steals too eagerly".)
- **Failure scenario:** dispatch A acquires ticket T, begins a 20-minute worker. At 5s, dispatch B considers the lock stale, takes it, and spawns a second worker on T. Two workers mutate the same scope → conflicting commits, doubled cost, corrupted ledger.
- **Fix:** set the staleness default to safely exceed a realistic worker lifetime (align with `worker_timeout`, currently 1200s in `config.yaml`, plus a margin) — a lock should be considered stale only when the owning process is provably dead (tie to B7's live-state recheck) or the worker-timeout deadline has passed, not on a fixed short timer.
- **AC:** M2-AC-BUG-4 (extended: also assert a held lock is NOT stolen within a normal worker lifetime).
...
| A-CLEAN-12 | `breaker.ts` | `stallCount`/`iterationCount` are written but never read (orthogonal review) — either wire them into the convergence-vs-attrition decision (B5) or drop; do not leave written-but-dead state |
| A-CLEAN-13 | `microverse.ts` | `MicroverseRunner` constructs a circuit breaker but never advances/records into it — the `canExecute` guard and breaker-tripped exit are inert. Resolve in B5 (real loop must record iterations into the breaker) |
...
- **C1 (AC-16 for real — the reference is a tautology):** `orchestrator/test/conformance/conformance-runner.test.ts` runs the new core directly and via a CLI that calls the same core — it never runs the legacy TS reference AC-16 requires (Codex CX-19). **Worse (orthogonal review, HIGH):** the `legacy-verdicts.json` baseline the differential is supposed to compare against was itself **generated by copying the new core's own output** (`legacy-reference-runner.js`). So the "differential conformance" test compares the core to a snapshot of itself — a tautology that passes by construction and would keep passing through any shared regression. Fix: obtain or reconstruct the *actual* legacy Pickle Rick TS reference (from `pickle-rick-claude` at the pinned commit), run every fixture through it and the new core independently, and diff complete typed outputs; only decision-backed deviations are allowed. If a true legacy reference cannot be reconstructed for a given predicate, that fixture's AC-16 claim must be marked unverifiable rather than silently green.
...
- **9-lens orthogonal agent team** (142 agents) with adversarial verification (3 refutation skeptics per finding) — one lens per goal + decision-conformance, security/blast-radius, test quality, TS correctness, Python fail-closed; **26 findings survived verification**, independently confirmed the policy-attachment gap and correctly calibrated the security defects as latent-not-live. It also contributed findings not surfaced by the other two reviews, folded in above: the chained/prefixed-command force-push bypass (A-SEC-1), the `shell`/`Shell` tool-name fail-open (A-SEC-3), the phantom-caller allowlist (A-SEC-6), the 5-second lock-steal (A-BUG-9), the tautological AC-16 reference (C1), and the dead breaker/microverse state (A-CLEAN-12/13).
