...
The preflight revalidated the t37 npm tarball and Python wheel against the packed-install receipt, then created a digest-keyed persistent installation under a proof-owned temporary root. The installed CLI reported build `ea8e5f7b4dde9311067c9dfafb66ea95e0817903`. The receipt binds hashes for the installed CLI, manager, worker, and policy package, plus the observed Omnigent Git identity.
