...
- **Description:** Execute four ordered gates. **t22A state bridge:** cut lifecycle/oracle/context authority to t18 ownership; add target-never-released, nonterminal cleanup-eligibility, failure-cleanup, promotion-cleanup, and quarantine proof types; remove the cleanup/oracle/promotion cycle. **t22B containment:** integrate an unforgeable authority-owned launch/membership/kill/empty boundary before target release. **t22C runner:** compose the exact success/failure state machines. **t22D cutover:** replace the legacy run worktree, direct Dispatcher spawn, caller-checkout gates, and finally release. Unavailable containment is a pre-release infrastructure error. macOS success requires a validated external privileged/VM authority unless support is explicitly narrowed; process groups, launchd, Seatbelt, and sampled ancestry do not qualify.
- **Declared Paths:** `orchestrator/src/process/containment.ts`, `orchestrator/src/process/supervisor.ts`, `orchestrator/src/lifecycle/attempt-runner.ts`, `orchestrator/src/lifecycle/cleanup.ts`, `orchestrator/src/context/execution-context.ts`, `orchestrator/src/policy/policy-bundle.ts`, `orchestrator/src/state/migrations.ts`, `orchestrator/src/state/oracle.ts`, `orchestrator/src/state/store.ts`, `orchestrator/src/state/transitions.ts`, `orchestrator/src/dispatch/dispatch.ts`, `orchestrator/src/git/run-workspace.ts`, `orchestrator/src/lifecycle/build.ts`, `orchestrator/test/reliability/attempt-critical-section.test.ts`
