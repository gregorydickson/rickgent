...
**Nested `-c` shell invocations (VAL-SEC-060).** A write hidden inside the quoted `-c` argument of a nested shell (`sudo -u root sh -c "echo x > out.txt"`, `bash -c 'truncate -s0 f'`) is not an inert literal — the redirect/write lives inside the `-c` string. When a segment's command word is a nested shell (`sh`, `bash`, `zsh`, `dash`, `ksh`, `ash`, `mksh`), `_shell_command_writes` extracts the `-c` argument (both `-c <cmd>` and short-flag bundles ending in `c`, e.g. `sh -ec <cmd>`) and evaluates it recursively with the same write-detection (redirects, interpreter one-liners, write utilities, disk-mutating git subcommands). A nested write → DENY; a provably read-only nested `-c` string (`sh -c "ls -la"`) is NOT false-DENIED. Recursion is bounded (depth > 24 fails closed) to defeat pathologically deep nesting. Verified by `test/test_scope_fence_nested_shell.py` (VAL-SEC-060) across every shell tool-name variant, authored red-first.
