...
4. **The AC-shape gate's normative component is better as a prompt instruction.** The `pickle-prd` skill (`.claude/commands/pickle-prd.md`) already instructs the agent to produce parametrized ACs — it requires each to be a "machine-checkable criterion" carrying a Verification clause — and `pickle-refine-prd` decomposes them into atomic tickets. The `citadel` skill verifies them post-implementation. A blocking gate that checks AC shape before any work begins is the exact false-block pattern that caused LOA-727 (a `title`-only quantifier false-rejected ~30 min + ~9 worker quotas per incident).
...
- **Status:** FIXED — countersign finding accepted. Both errors were factual, not interpretive.
- **LOC figures:** 57,117 and 26,521 were **byte counts mislabeled as LOC** (off by ~40×). Corrected against the pinned snapshot: `check-readiness.ts` is **1,338 LOC** (57,117 bytes) and `audit-ticket-bundle.ts` is **720 LOC** (26,521 bytes). The byte counts match the claimed figures exactly, which confirms the mislabeling.
- **`prism` skill:** does not exist — a search of the pinned snapshot returns nothing. Replaced with the real skill, `pickle-prd` (`.claude/commands/pickle-prd.md`), which already mandates a "machine-checkable criterion" with a Verification clause (test/typecheck/lint/curl/llm-conformance); `pickle-refine-prd` handles decomposition. Corrected in both the Evaluation and Reasoning sections.
- **Decision unchanged:** skip. Neither error touched the load-bearing argument — the gates are advisory per R-GATE-ADVISORY, and the pre-PR gate (tsc/eslint/tests) remains the machine-checkable enforcement surface. The verdict was APPROVED_WITH_NOTES, not REJECTED, for exactly this reason.
- **Date:** 2026-07-12
