...
| build\|pipeline --feature\|--no-autonomous-pr | public_local_artifact | local_artifact_only | automatic_delivery/enabled | delivery configuration accepted; verified push and idempotent PR creation are required before marking delivered | — | — | RICKGENT_DELIVERY_ACTIVE | Automatic delivery is activated (t34): branch push uses the exact delivery OID with independent ls-remote confirmation, and PR creation/resolution requires queried head OID and repository identity equality. Retries resolve the same PR without duplicates. The protected real-provider run remains in t38. |
